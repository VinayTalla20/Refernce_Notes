...
def getConnectorDetailsByCustomerId(communityId, from_time_ist, to_time_ist, access_token, tenant_id, customer_name):
    ...
    metrics_response = requests.get(url=metrics_url, headers=headers, verify="/app/icsservices.cer")
...
if __name__ == "__main__":
    ...
    excludeServiceAvailabilityCount = excludeServiceAvailbilityRequests(tenant_ids=["aarp/availability"], from_time_ist=front_door_from_time_ist, to_time_ist=front_door_to_time_ist)
    ...
    JMP_CLASSIC_REQUEST_COUNT = getFrontDoorRequestCount(tenant_ids=["/posttoclassic/x508qy48MTA1OA=="], from_time_ist=front_door_from_time_ist, to_time_ist=front_door_to_time_ist)
